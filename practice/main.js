...
function dailySeed() {
    const now = new Date();
    return now.getDate() + Math.floor(Math.random()* 12 * 32) + Math.floor(((Math.random()*50) + 2000)*400);
}
...
function save() {
    localStorage.setItem("diffle_play_practice", JSON.stringify(play));
    localStorage.setItem("diffle_stats_practice", JSON.stringify(stats));
}
function load() {
    const today = getTodayString();
    const statsString = localStorage.getItem("diffle_stats_practice");
    ...
    const playString = localStorage.getItem("diffle_play_practice");
...
function shareImage() {
    ...
    context.textAlign = "center";
    context.fillText("Practice", width / 2, 65);
...
document.getElementById("play_again_button").onclick = e => {
